...
function renderDifficultes(container) {
  const list = getDifficultes().filter(d => d.seen);
  container.innerHTML = `
    <section class="page page--difficultes">
      <h1 class="page__title">❌ Mes difficultés</h1>
      <p class="page__subtitle">Classées de la moins maîtrisée à la mieux maîtrisée, uniquement à partir des exercices que tu as déjà faits.</p>
      ${list.length ? `<div class="difficultes-list">
        ${list.map(d => `
          <div class="difficulte-row">
            ...
            <div class="difficulte-row__score">${d.score}%</div>
      ...
      </div>` : `<p class="card__empty">Rien à afficher pour l'instant — fais quelques exercices, et les notions à retravailler apparaîtront ici automatiquement, classées selon tes vrais résultats.</p>`}
    </section>`;
}
